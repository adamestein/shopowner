...
def impl(context, page):
    if page == "Edit Sale":
        context.browser.visit(context.config.server_url + "/shopowner/sales/edit/")
...
def impl(context, thing, action):
    if thing == "sale":
        ...
        if action == "recorded":
            ...
            assert sale.item == context.item1
            assert sale.tax_rate == 8.0
            assert sale.discount == 20.0
            assert almost_equal(sale.price, decimal.Decimal(1.06), 0.000001)
            assert almost_equal(sale.commission, decimal.Decimal(0.06), 0.000001)
            assert str(sale.date) == "2014-11-15"
        else:
            # Unknown action
            assert False
    else:
...
def impl(context, what):
    if what == "tax rate":
        assert context.browser.is_text_present(
            "Can't automatically calculate price or commission without the tax rate.")
    else:
        # Unknown what
        assert False
